...
import Auth from "./components/Auth"; // <-- NEW: Import the Auth Gateway
...
function App() {
  ...
  // Check browser memory on first load to see if they are already logged in
  useEffect(() => {
  ...
  useEffect(() => {
    // Only fetch movies if the user is actually logged in!
  ...
  // --- THE GATEKEEPER ---
  // If not authenticated, completely hide the app and show the Auth screen
  if (!isAuthenticated) {
    ...
    <div className="min-h-screen bg-brand-dark font-sans text-brand-text selection:bg-brand-accent selection:text-white pb-20 relative">
      <nav className="bg-black/80 backdrop-blur-xl border-b border-white/5 py-4 sticky top-0 z-50">
        <div className="max-w-7xl mx-auto px-6 flex justify-between items-center">
          ...
          {/* NEW: User Profile & Disconnect Button */}
